...
export default function Page() {
  ...
  const bookHandler = (data : {date : string, time : string[]}) => {
    ...
    if( !category || !sessions || !client || !user || !artist) return errorAlert("empty field")
  ...
  return (
    <div className="w-4/6 mx-auto py-10 space-y-6">
      ...
      <div className="space-y-6">
                  ...
                  <Select onValueChange={setArtist}>
                    ...
                    <SelectContent>
                      {bussinessInfoData?.artists.map((artist) => (
                        <SelectItem  key={artist.artist._id} value={artist.artist._id}> <img src={artist.artist.profile} className="w-5 h-5 object-cover rounded-full" />  {artist.artist.name} </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
        ...
        <BookModal key={artist}  artistId={artist}   sessionTime={sessions[0]}  callBack={bookHandler}/>
